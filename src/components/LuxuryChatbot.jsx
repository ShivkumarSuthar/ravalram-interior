      ...
      <AnimatePresence>
        {showNotification && !isOpen && (
          <motion.div
            ...
            className="absolute bottom-20 right-0 w-72 bg-stone-900 border border-gold-accent/30 text-white p-4 shadow-xl z-20"
          >
            <button 
            ...
            <div className="flex items-start space-x-3 pr-4">
              <div className="w-8 h-8 rounded-full bg-gold-accent/10 border border-gold-accent flex items-center justify-center shrink-0 mt-0.5 text-gold-accent text-xs font-bold">
                P
              </div>
            ...
            <div className="mt-3 flex justify-end">
              <button 
                ...
                className="text-[10px] tracking-widest font-bold text-gold-accent hover:text-white transition-colors uppercase inline-flex items-center space-x-1 cursor-pointer"
              >
        ...
        <AnimatePresence mode="wait">
          {isOpen ? (
            ...
            >
              ...
              <span className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-gold-accent border-2 border-stone-950 animate-ping" />
            </motion.div>
          )}
        </AnimatePresence>
      </motion.button>
          ...
          >
            ...
            <div className="bg-stone-900 border-b border-white/5 p-4 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className="relative">
                  <div className="w-10 h-10 rounded-full bg-stone-800 border border-gold-accent/40 flex items-center justify-center text-gold-accent font-bold text-base">
                    P
                  </div>
                  <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-gold-accent rounded-full border-2 border-stone-900 shadow-[0_0_6px_var(--color-gold-accent)]" />
                </div>
                <div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin bg-gradient-to-b from-stone-950 via-stone-950 to-stone-900">
              {messages.map((msg) => {
                ...
                return (
                  ...
                  >
                    {isBot ? (
                      <div className="w-7 h-7 rounded-full bg-stone-800 border border-gold-accent/20 flex items-center justify-center text-gold-accent text-[10px] font-bold shrink-0">
                        P
                      </div>
                    ) : (
                      <div className="w-7 h-7 rounded-full bg-gold-accent/10 border border-gold-accent/20 flex items-center justify-center text-gold-accent shrink-0">
                        <User size={12} />
                      </div>
                    )}
                    ...
                    <div className="space-y-1 max-w-[75%]">
                      <div className={`p-3 text-xs leading-relaxed font-light select-none ${
                        isBot 
                          ...
                          : "bg-gold-accent text-stone-950 font-medium"
                      }`}>
                        {msg.text}
                      </div>
              ...
              {isTyping && (
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="bg-stone-900/80 border border-white/5 p-3 px-4 flex items-center space-x-1 max-w-[75%]">
                    <span className="w-1.5 h-1.5 rounded-full bg-gold-accent/60 animate-bounce delay-100" />
                    <span className="w-1.5 h-1.5 rounded-full bg-gold-accent/60 animate-bounce delay-200" />
                    <span className="w-1.5 h-1.5 rounded-full bg-gold-accent/60 animate-bounce delay-300" />
                  </div>
                </div>
              )}
            ...
            <div className="px-4 py-2 border-t border-white/5 bg-stone-950/80 overflow-x-auto whitespace-nowrap flex gap-2 no-scrollbar">
              {PRESETS.map((preset, idx) => (
                <button
                  ...
                  className="px-3 py-1.5 bg-stone-900 hover:bg-stone-800 text-gold-accent hover:text-white border border-white/5 hover:border-gold-accent text-[10px] font-mono tracking-wide uppercase transition-all cursor-pointer inline-block rounded-none shrink-0"
                >
                  {preset.keywords[0]} ?
                </button>
              ))}
            ...
            <div className="bg-stone-950 py-1.5 px-4 border-t border-white/5 text-[9px] text-stone-600 font-mono tracking-widest uppercase flex items-center justify-between select-none">
              ...
              <div className="flex items-center space-x-1 text-gold-accent/60 font-serif lowercase">
                <span>suthar</span>
                <span className="w-1 h-1 bg-gold-accent rounded-full" />
              </div>
            </div>
